...
func _ready() -> void:
	...
	_animated_sprite.play("Enemigo03")  # Suponiendo que "Idle" es la animación de reposo del enemigo
...
func persigue_a_jugador(objetivo: Node2D) -> void:
	...
	if velocity.length() > 0:
		_animated_sprite.play("Enemigo03")  # Reproducir animación de caminar
	else:
		_animated_sprite.play("Enemigo03")  # Si no se mueve, reproducir animación de reposo
